...
{
	//SR_Fence deletionFence = SR_RenderDevice::gInstance->InsertFence(SR_CommandListType::Graphics);
	//EndFrameQueueAndList(mTextureKeepAliveList, mTempTextureRemovalQueue, SR_Fence());
	mTextureKeepAliveList.RemoveAll();
	EndFrameInternal();
}

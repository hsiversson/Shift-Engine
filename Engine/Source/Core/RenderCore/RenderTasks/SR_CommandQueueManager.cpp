...
void SR_CommandQueueManager::InsertFence(const SR_CommandListType& aCommandListType, SR_Fence& aFence)
{
	aFence.mType = aCommandListType;
	mCommandQueueThreads[static_cast<uint32>(aCommandListType)]->InsertFence(aFence);
}

void SR_CommandQueueManager::InsertWait(const SR_Fence& /*aFence*/, const SR_CommandListType& /*aCommandListType*/)
{
}
...
SR_CommandQueueManager::CommandQueueThread::CommandQueueThread(const SR_CommandListType& aCommandListType)
	...
	while (mIsRunning)
	{
		...
		while (!localWorkQueue.empty())
		{
			...
			else if (work.mFuture.IsReady())
			{
				...
				{
					cmdList->End();
					cmdListsToSubmit.Add(cmdList.get());// We can schedule for GPU submission
...
void SR_CommandQueueManager::CommandQueueThread::SubmitTask(InFlightTaskSignature aTask, SR_TaskEvent* aOutEvent)
{
...
void SR_CommandQueueManager::CommandQueueThread::InsertFence(const SR_Fence& aFence)
{
	CommandQueueWork work;
	work.mType = CommandQueueWork::Type::SignalFence;
	work.mFence = aFence;
...
SC_Ref<SR_CommandList> SR_CommandQueueManager::CommandQueueThread::GetCommandList()
{
...
void SR_CommandQueueManager::CommandQueueThread::SubmitCommandLists(SC_Array<SR_CommandList*>& aCommandLists, SC_Array<CommandQueueWork>& aWorkToSignal)
{

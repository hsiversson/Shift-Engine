...
class SR_CommandList_Vk final : public SR_CommandList
{
public:
	SR_CommandList_Vk(const SR_CommandListType& aType);
	~SR_CommandList_Vk();
	...
	SR_BufferResource* GetBufferResource(uint64& aOutOffset, SR_BufferBindFlag aBufferType, uint32 aByteSize, void* aInitialData, uint32 aAlignment = 0, const SR_Fence& aCompletionFence = SR_Fence()) override;
	SR_Buffer* GetBuffer() override;
